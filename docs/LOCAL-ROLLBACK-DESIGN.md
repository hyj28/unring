...
| 24 | Finalization status is presented in user terms without weakening the audit. Completed empty scans state zero explicitly; interrupted `run`, `log`, and `restore` views state that the list is incomplete and never render it as clean, while structured JSON retains the exact cause. The 50-row change bound applies **per watched root** as well as to declared agent-own-state. Automatic retention groups shown IDs and states its accounting caveat once. History display distinguishes `no decision`, explicit `discarded`, and abnormal `interrupted`/`abnormal end` while preserving the stored outcome. A second post-child signal is acknowledged, the first signal remains authoritative for the exit status, and durable seal-and-discard finalization continues so another signal cannot create a pending or unsealed record. |
| 25 | Post-child signal ownership is continuous through scan, retention, interceptor sealing, decision, and finalization; one coordinator cancels the active phase and forces a recorded abnormal discard. Retention lock waits, planning, and recursive clone deletion accept cancellation. A canceled filesystem walk is a scan failure, never a claim that the already-created clone snapshot was missing, and interruption is set before the sealed manifest's first publication. `tmutil` batch path comparison uses canonical Unicode equivalence, whitespace/line-break paths are isolated, and every unusable batch retries per path so one malformed answer cannot erase its neighbors or misname the failure. Excluded inheritance walks only the candidate's ancestors. Human bounds apply separately to every declared agent-state root and every top-level outside-watch presentation root without changing the manifest. |
| 26 | Signal ownership begins immediately after the durable audit record is opened and covers baseline capture, pre-child retention, the child, and every finishing phase; `SIGPIPE` from an ordinary closed output pipe follows the same recorded abnormal-discard path as `SIGINT` and `SIGTERM`. Pre-child retention emits periodic progress. An incomplete watched-root after-scan is explicitly unavailable and contributes no entries to the diff, because absence from a partial walk is not evidence of deletion. Progress writes and the interrupt acknowledgement share an ordering gate so no progress line can appear after the stop message. |
| 27 | A signal received during the child fixes the run's abnormal-discard disposition and is forwarded to the child, but the post-session filesystem scan starts on a fresh cancellation boundary so clone evidence remains discoverable and restorable. A signal received after that boundary cancels the scan. `SIGHUP` uses the same durable path. A clone-path root scan that cannot run is excluded from the clone diff, while a separately completed wide scan may still contribute its independently observed changes. Live review, stored review, restore listing, and restore execution all disclose unscanned roots. |
| 28 | Parse `tmutil isexcluded` using its real two-space status separator, consuming only that delimiter so legitimate leading and trailing path spaces remain part of the echoed path. A reviewable session rolled back by the documented non-interactive default records a distinct completion kind and displays as `default discard`; only signals, crashes, incomplete coverage, and other abnormal paths display as abnormal. |
| 29 | Refine decision 28's broad incomplete-coverage rule: file coverage and completion disposition are separate dimensions. OS-classified permission refusals count as routine only when they are the complete failure set of the widened background scan; they remain stored and rendered as incomplete coverage but do not make an otherwise normal session display as abnormal. The classification is recorded at the filesystem error boundary, not inferred from human error text. Permission failures within watched roots, unscanned roots, interruptions, persistence errors, and mixed or unknown failures remain abnormal. |

These decisions alter process count, cancellation, audit presentation metadata, and rendering only. Diff membership,
